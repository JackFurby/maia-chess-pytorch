...
for i in {1000..2000..100}; do
    ...
    mkdir $outputdir
    for f in ../data/lichess_raw/lichess_db_standard_rated_2014-{01..12}.pgn.bz2; do
    #for f in ../data/lichess_raw/lichess_db_standard_rated_2017* ../data/lichess_raw/lichess_db_standard_rated_2018* ../data/lichess_raw/lichess_db_standard_rated_2019-{01..11}.pgn.bz2; do
        fname="$(basename -- $f)"
        echo "${i}-${fname}"
        screen -S "${i}-${fname}" -dm bash -c "python3 replication-extractELOrange.py --remove_bullet --remove_low_time ${i} ${upperval} ${outputdir}/${fname} ${f}"
...
while screen -list | grep lichess
do
    sleep 1
done
...
for i in {1000..2000..100}; do
    ...
    #for y in {2017..2019}; do
        ...
        mkdir $y
        cd $y
        screen -S "${i}-${y}-pgns" -dm bash -c "source ~/.bashrc; bzcat \"../../../pgns_ranged_filtered/${i}/lichess_db_standard_rated_${y}\"* | pgn-extract -7 -C -N  -#200000"
